...
import net.miaohy.pb.common.utils.IniUtil;
import net.miaohy.pb.config.shiro.jwt.JwtCredentialsMatcher;
import net.miaohy.pb.config.shiro.jwt.JwtFilter;
import net.miaohy.pb.config.shiro.jwt.JwtProperties;
import net.miaohy.pb.config.shiro.jwt.JwtRealm;
import net.miaohy.pb.exception.PbException;
import net.miaohy.pb.filter.RequestPathFilter;
...
public class ShiroConfig {
     ...
     */
    @Bean
    public JwtRealm jwtRealm(PbUserIdentityService pbUserIdentityService) {
        log.info("============================ShiroConfig jwtRealm========================================");
        JwtRealm jwtRealm = new JwtRealm(pbUserIdentityService);
    ...
    public DefaultSubjectDAO subjectDAO() {
        log.info("============================ShiroConfig subjectDAO========================================");
     ...
     */
    @Bean
    public SecurityManager securityManager(PbUserIdentityService pbUserIdentityService) {
        log.info("============================ShiroConfig securityManager========================================");
        DefaultWebSecurityManager securityManager = new DefaultWebSecurityManager();
        securityManager.setRealm(jwtRealm(pbUserIdentityService));
    ...
    public ShiroFilterFactoryBean shiroFilterFactoryBean(SecurityManager securityManager,
                                                         ...
                                                         PbUserIdentityService pbUserIdentityService,
                                                         ShiroFilterProperties filterProperties,
                                                         ShiroProperties shiroProperties,
                                                         JwtProperties jwtProperties) {
        log.info("============================ShiroConfig shiroFilterFactoryBean========================================");
        ShiroFilterFactoryBean shiroFilterFactoryBean = new ShiroFilterFactoryBean();
        shiroFilterFactoryBean.setSecurityManager(securityManager);
        Map<String, Filter> filterMap = getFilterMap(loginserviceImpl, pbUserIdentityService, filterProperties, jwtProperties);
     ...
     * @return
     */
    private Map<String, Filter> getFilterMap(LoginService loginserviceImpl, PbUserIdentityService pbUserIdentityService,
                                             ShiroFilterProperties filterProperties,
                                             JwtProperties jwtProperties) {
        Map<String, Filter> filterMap = new LinkedHashMap();
        filterMap.put(REQUEST_PATH_FILTER_NAME, new RequestPathFilter(filterProperties.getRequestPath()));
        filterMap.put(JWT_FILTER_NAME, new JwtFilter(loginserviceImpl, pbUserIdentityService, jwtProperties));
        return filterMap;
    }
    ...
    private Map<String, String> getFilterChainDefinitionMap(ShiroProperties shiroProperties) {
        log.info("============================ShiroConfig getFilterChainDefinitionMap========================================");
        ...
        if (CollectionUtils.isNotEmpty(permissionConfigs)) {
            for (ShiroPermissionProperties permissionConfig : permissionConfigs) {
                ...
                if (StringUtils.isBlank(url) && ArrayUtils.isEmpty(urls)) {
                    throw new PbException("shiro permission config 路径配置不能为空");
                }
                if (StringUtils.isBlank(permission)) {
                    throw new PbException("shiro permission config permission不能为空");
                }
    ...
    @Bean
    public Authenticator authenticator(PbUserIdentityService pbUserIdentityService) {
        ModularRealmAuthenticator authenticator = new ModularRealmAuthenticator();
        authenticator.setRealms(Arrays.asList(jwtRealm(pbUserIdentityService)));
